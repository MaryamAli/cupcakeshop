class CommentUpdateJob < ApplicationJob
  ...
  def perform(comment, current_user)
    # Do something later
    #from comments_controller for 6.14
    ProductChannel.broadcast_to(comment.product_id, comment: 
    render_comment(comment, current_user), average_rating: comment.product.average_rating)
  end
